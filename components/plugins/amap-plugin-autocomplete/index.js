...
import windowVal from "../../../brick/utils"
...
class Autocomplete extends Component {
  ...
  loadWithOptions(props){
    ...
    this.map.plugin(['AMap.Autocomplete', 'AMap.Geocoder'], () => {
      ...
      const auto = new windowVal.winVal.AMap.Autocomplete(opts);
      this.map.addControl(auto);
        windowVal.winVal.AMap.event.addListener(auto, "select", this.onSelect);//注册监听，当选中某条记录时会触发
      const geoloc = new windowVal.winVal.AMap.Geocoder({});
      console.log(geoloc);
